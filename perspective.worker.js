 ...
 *
 */const DEFAULT_CONFIG=(__webpack_require__(306)/* ["default"] */ .Z);module.exports.get_types=function(){return Object.keys(module.exports.get_config().types);};module.exports.get_type_config=function(type){const config={};if(module.exports.get_config().types[type]){Object.assign(config,module.exports.get_config().types[type]);}if(config.type){const props=module.exports.get_type_config(config.type);Object.assign(props,config);return props;}else{return config;}};function isObject(item){return item&&typeof item==="object"&&!Array.isArray(item);}function mergeDeep(target,...sources){if(!sources.length)return target;const source=sources.shift();if(isObject(target)&&isObject(source)){for(const key in source){if(isObject(source[key])){if(!target[key])Object.assign(target,{[key]:{}});mergeDeep(target[key],source[key]);}else{Object.assign(target,{[key]:source[key]});}}}return mergeDeep(target,...sources);}globalThis.__PERSPECTIVE_CONFIG__=undefined;module.exports.override_config=function(config){if(globalThis.__PERSPECTIVE_CONFIG__){console.warn("Config already initialized!");}globalThis.__PERSPECTIVE_CONFIG__=mergeDeep(DEFAULT_CONFIG,config);};module.exports.get_config=function get_config(){if(!globalThis.__PERSPECTIVE_CONFIG__){globalThis.__PERSPECTIVE_CONFIG__=mergeDeep(DEFAULT_CONFIG,globalThis.__TEMPLATE_CONFIG__||{});}return globalThis.__PERSPECTIVE_CONFIG__;};
 ...
 */if(!String.prototype.includes){String.prototype.includes=function(search,start){if(typeof start!=="number"){start=0;}if(start+search.length>this.length){return false;}else{return this.indexOf(search,start)!==-1;}};}/* eslint-disable-next-line max-len */ // from https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/includes
 ...
 */class Server{constructor(perspective){this.perspective=perspective;this._tables={};this._views={};this._callback_cache=new Map();}/**
     ...
     */init(msg){if(msg.config){(0,config.override_config)(msg.config);}// The client will wait for a response message on table() and
...
this.process_error(msg,{message:"View method cancelled"});return;}if(obj&&obj.push){obj.push(msg);return;}try{if(msg.subscribe){this.process_subscribe(msg,obj);return;}else{result=obj[msg.method].apply(obj,msg.args);if(result instanceof Promise){result.then(result=>this.process_method_call_response(msg,result)).catch(error=>this.process_error(msg,error));}else{this.process_method_call_response(msg,result);}}}catch(error){this.process_error(msg,error);return;}}/**
     ...
     */process_error(msg,error){this.post({id:msg.id,error:error_to_json(error)});}/**
...
;// CONCATENATED MODULE: ../packages/perspective/src/js/view_formatters.js
/******************************************************************************
 *
 * Copyright (c) 2017, the Perspective Authors.
 *
...
let extracted=[];for(let i=0;i<vector.size();i++){let item=vector.get(i);extracted.push(col_path_vector_to_string(item));}vector.delete();return extracted;};/**
     ...
     */view.prototype.schema=function(override=true){const schema=extract_map(this._View.schema());if(override){for(const key of Object.keys(schema)){let colname=key.split(COLUMN_SEPARATOR_STRING);colname=colname[colname.length-1];if(this.overridden_types[colname]&&(0,config.get_type_config)(this.overridden_types[colname]).type===schema[key]){schema[key]=this.overridden_types[colname];}}}return schema;};/**
...
value=new Date(value);value=value.toLocaleString("en-us",type_config.format);}}formatter.setColumnValue(data,row,col_name,value);}}if(get_pkeys){const keys=slice.get_pkeys(ridx,0);formatter.initColumnValue(data,row,"__INDEX__");for(let i=0;i<keys.size();i++){// TODO: if __INDEX__ and set index have the same value,
// don't we need to make sure that it only emits one?
const s=keys.get(i);const value=__MODULE__.scalar_to_val(s,false,false);s.delete();formatter.addColumnValue(data,row,"__INDEX__",value);}keys.delete();}// we could add an api to just clone the index column if
// it's already calculated
if(get_ids&&num_sides===0){const keys=slice.get_pkeys(ridx,0);for(let i=0;i<keys.size();i++){const s=keys.get(i);const value=__MODULE__.scalar_to_val(s,false,false);s.delete();formatter.addColumnValue(data,row,"__ID__",value);}keys.delete();}if(row_path){row_path.delete();}formatter.addRow(data,row);}slice.delete();return formatter.formatData(data,options.config);};/**
...
let slice,data_slice;if(!options.data_slice){data_slice=this.get_data_slice(start_row,end_row,idx,idx+1);slice=data_slice.get_slice();}else{slice=options.data_slice.get_column_slice(idx);}const dtype=this._View.get_column_dtype(idx);const rst=format_function(slice,dtype,idx);slice.delete();if(data_slice){data_slice.delete();}return rst;};/**
     ...
     */view.prototype.to_columns=function(options){return to_format.call(this,options,view_formatters.jsonTableFormatter);};/**
     ...
     * supplied, the keys of this object will be comma-prepended with their
     * comma-separated column paths.
     */view.prototype.to_json=function(options){return to_format.call(this,options,view_formatters.jsonFormatter);};/**
     ...
     */view.prototype.to_arrow=function(options={}){_call_process(this.table.get_id());options=_parse_format_options.bind(this)(options);const start_row=options.start_row;const end_row=options.end_row;const start_col=options.start_col;const end_col=options.end_col;const sides=this.sides();if(this.is_unit_context){return __MODULE__.to_arrow_unit(this._View,start_row,end_row,start_col,end_col);}else if(sides===0){return __MODULE__.to_arrow_zero(this._View,start_row,end_row,start_col,end_col);}else if(sides===1){return __MODULE__.to_arrow_one(this._View,start_row,end_row,start_col,end_col);}else if(sides===2){return __MODULE__.to_arrow_two(this._View,start_row,end_row,start_col,end_col);}};/**
     ...
     */view.prototype.num_columns=function(){const ncols=this._View.num_columns();const nhidden=this._num_hidden();return ncols-ncols/(this.config.columns.length+nhidden)*nhidden;};/**
...
callback(updated);}});};function filterInPlace(a,condition){let i=0,j=0;while(i<a.length){const val=a[i];if(condition(val,i,a))a[j++]=val;i++;}a.length=j;return a;}/*
     ...
     */table.prototype.clear=function(){_remove_process(this.get_id());this._Table.reset_gnode(this.gnode_id);};/**
...
for(const callback of this._delete_callbacks){callback();}};/**
     ...
     */table.prototype.size=function(){_call_process(this._Table.get_id());return this._Table.size();};/**
...
filter[2]=new Date(filter[2]);}}}let name=Math.random()+"";let sides;if(config.group_by.length>0||config.split_by.length>0){if(config.split_by&&config.split_by.length>0){sides=2;}else{sides=1;}}else{sides=0;}let vc=new view_config(config);let v=new view(this,sides,config,vc,name);this.views.push(v);return v;};/* eslint-enable max-len */let meter;function initialize_profile_thread(){if(meter===undefined){let _msgs=0;let start=performance.now();setTimeout(function poll(){let now=performance.now();console.log(`${(1000*_msgs/(now-start)).toFixed(2)} msgs/sec`);_msgs=0;start=now;setTimeout(poll,5000);},5000);meter=function update(x){_msgs+=x;};console.log("Profiling initialized");}}/**
...
return new table(_Table,options.index,options.limit,overridden_types);}catch(e){if(_Table){_Table.delete();}console.error(`Table initialization failed: ${e}`);throw e;}}};for(let prop of Object.keys(constants_namespaceObject)){perspective[prop]=constants_namespaceObject[prop];}/**
     ...
     */class WebWorkerServer extends Server{/**
         ...
         */init(msg){if(typeof WebAssembly==="undefined"){throw new Error("WebAssembly not supported");}else{__MODULE__({wasmBinary:msg.buffer,wasmJSMethod:"native-wasm"}).then(mod=>{__MODULE__=mod;__MODULE__.init();notify_main_thread();super.init(msg);});}}}/**
     * Use WebSorkerServer as default inside a Web Worker, where `window` is
     * replaced with `self`.
     */if(typeof self!=="undefined"&&self.addEventListener){new WebWorkerServer(perspective);}return perspective;}function notify_main_thread(){if(typeof self!=="undefined"){try{if(self.dispatchEvent&&!self._perspective_initialized&&self.document!==null){self._perspective_initialized=true;const event=self.document.createEvent("Event");event.initEvent("perspective-ready",false,true);self.dispatchEvent(event);}else if(!self.document&&self.postMessage){self.postMessage({});}}catch(e){}}}
...
if (globalThis.document !== undefined && typeof WebAssembly !== "undefined") {
    ...
    _perspective_instance = globalThis.perspective =
        perspective(perspective_cpp);
}

/* harmony default export */ const perspective_worker = ((/* unused pure expression or super */ null && (_perspective_instance)));
