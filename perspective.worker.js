...
value=new Date(value);value=value.toLocaleString([],type_config.format);}}formatter.setColumnValue(data,row,col_name,value);}}if(get_pkeys){const keys=slice.get_pkeys(ridx,0);formatter.initColumnValue(data,row,"__INDEX__");for(let i=0;i<keys.size();i++){// TODO: if __INDEX__ and set index have the same value,
...
filter[2]=new Date(filter[2]);}}}let name=Math.random()+"";let sides;if(config.group_by.length>0||config.split_by.length>0){if(config.split_by&&config.split_by.length>0){sides=2;}else{sides=1;}}else{sides=0;}let vc=new view_config(config);let v=new view(this,sides,config,vc,name);this.views.push(v);return v;};let meter;function initialize_profile_thread(){if(meter===undefined){let _msgs=0;let start=performance.now();setTimeout(function poll(){let now=performance.now();console.log(`${(1000*_msgs/(now-start)).toFixed(2)} msgs/sec`);_msgs=0;start=now;setTimeout(poll,5000);},5000);meter=function update(x){_msgs+=x;};console.log("Profiling initialized");}}/**
